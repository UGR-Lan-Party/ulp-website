...
document.addEventListener('DOMContentLoaded', () => {
    ...
    for (let i = 0; i < numberOfPages; i++) {
        ...
        pageButton.className = 'rounded-full duration-300 bg-primary dark:bg-gray-400 text-white dark:text-primary hover:bg-secondary dark:hover:bg-gray-200 min-w-[40px] min-h-[40px] min-h-4 text-xl lg:text-2xl transition-all ease-in-out shadow-black';
        ...
        pageButton.addEventListener('click', () => {
            ...
            currentButton.classList.remove('bg-secondary', 'dark:bg-gray-200', 'shadow-md');
...
function typeTitleEffect(element, newText) {
    ...
    function replaceText() {
        if (index < maxLength) {
            ...
            let remainingPart = oldText.length > newText.length 
                ? oldText.substring(index + 1)
                : "";
